...
class DBAdapter:
    def __init__(
        ...
        self.__engine = create_engine(self.DB_URL, pool_size=10, encoding='utf-8', echo=False)
    ...
    def construct_new_scoped_session(self):
        # https://docs.sqlalchemy.org/en/14/orm/contextual.html?highlight=bulk_save#sqlalchemy.orm.scoping.scoped_session.__init__
        self.__session.__init__(sessionmaker(autocommit=False, autoflush=False, bind=self.__engine))
        return self.__session.__call__()
    ...
    def make_tables(self, tables: Optional[list[Any]] = None) -> None:
        if tables is None:
            tables = []
        # log.debug(
            #     f'base.metadata.create_all {','.join([table.__name__ for table in tables])}'
        # )
        args = {}
        if tables:
    ...
    def delete_tables(self, tables: Optional[list[Any]] = None) -> None:
        if tables is None:
            tables = []
        # log.debug(
            #     f'base.metadata.drop_all {','.join([table.__name__ for table in tables])}'
        # )
        args = {}
        if tables:
            args['tables'] = [table.__table__ for table in tables]
        BASE.metadata.drop_all(bind=self.__engine, **args)
    ...
    def is_exists(self, table: Any, **kwargs: Any) -> bool:
        return self.__session.query(table).filter_by(**kwargs).first() is not None

    def refresh_item(self, item):
        return self.__session.refresh(item)
